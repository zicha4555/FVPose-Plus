...
def main():
    ...
    if config.TEST.MODEL_FILE and os.path.isfile(test_model_file):
        ...
        model.module.load_state_dict(torch.load(test_model_file, map_location=torch.device(model.module.load_state_dict(torch.load(test_model_file, map_location=torch.device(f'cuda:{model.device_ids[0]}'))))))
    else:
