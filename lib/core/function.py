...
def train_3d(config, model, optimizer, loader, epoch, output_dir, writer_dict, device=torch.device('cuda')):
    ...
    # change
    for i, (inputs, targets, meta, input_heatmap) in enumerate(tqdm(loader, ncols=100)):
        ...
        if i % (len(loader) // config.PRINT_TIMES_PER_EPOCH) == 0:
...
def validate_3d(config, model, loader, output_dir, has_evaluate_function=False, device=torch.device('cuda')):
    ...
    # loading constants of the dataset
    cameras = loader.dataset.cameras
    resize_transform = loader.dataset.resize_transform
