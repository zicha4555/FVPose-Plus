...
class ProjectLayer(nn.Module):
    def __init__(self, cfg):
        ...
        self.register_buffer('whole_space_center', torch.tensor(cfg.CAPTURE_SPEC.SPACE_CENTER, device=self.device))
        self.register_buffer('whole_space_size', torch.tensor(cfg.CAPTURE_SPEC.SPACE_SIZE, device=self.device))
        self.register_buffer('ind_space_size', torch.tensor(cfg.INDIVIDUAL_SPEC.SPACE_SIZE, device=self.device))
        self.register_buffer('voxels_per_axis', torch.tensor(cfg.INDIVIDUAL_SPEC.VOXELS_PER_AXIS, device=self.device, dtype=torch.int32))
        self.register_buffer('fine_voxels_per_axis', (self.whole_space_size / self.ind_space_size * (self.voxels_per_axis - 1)).int() + 1)

        self.register_buffer('scale', (self.fine_voxels_per_axis.float() - 1)  / self.whole_space_size)
    ...
    def project_grid(self, camera, w, h, nbins, resize_transform, device):
        ...
        xy = do_transform(xy, torch.as_tensor(resize_transform, dtype=torch.float, device=device))
        xy = xy * torch.tensor(
    ...
    '''
    def compute_sample_grid(self, heatmaps, meta, i, voxels_per_axis, cameras, resize_transform):
    ...
    def forward(self, heatmaps, index, meta, proposal_centers, cameras, resize_transform):
        ...
        if curr_seq not in self.sample_grids:
            ...
            sample_grids = self.sample_grids[curr_seq].to(device)
        ...
        del sample_grids, centers_tl, mask, start, end
        return cubes, offset