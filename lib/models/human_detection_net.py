...
class ProposalLayer(nn.Module):
    def __init__(self, cfg):
        ...
        self.register_buffer('scale', (torch.tensor(cfg.CAPTURE_SPEC.SPACE_SIZE) / (torch.tensor(cfg.CAPTURE_SPEC.VOXELS_PER_AXIS) - 1)).to(self.device))
        self.register_buffer('bias', (torch.tensor(cfg.CAPTURE_SPEC.SPACE_CENTER) - torch.tensor(cfg.CAPTURE_SPEC.SPACE_SIZE) / 2.0).to(self.device))
